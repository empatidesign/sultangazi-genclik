...
class FastMenuManagementNestable {
	...
	public function get_nestable($ci_result) {
		if (isNotNull($ci_result)) {
			foreach ($ci_result as $data) {
				...
				$this->ci_arr_data[$data->menu_id]['edit'] = '<a href="'.BACKEND_URL.'/'.ADMIN_URL_DESIGNS.'/'.ADMIN_URL_FAST_MENU_MANAGEMENT.'/edit/'.$data->menu_id.'" class="float-right mr-1">'.set_primary(lang('Admin.process.edit')).'</a>';
				$this->ci_arr_data[$data->menu_id]['delete'] = '<a href="javascript:void(0)" class="float-right customDelete" data-url="'.BACKEND_URL.'/'.ADMIN_URL_DESIGNS.'/'.ADMIN_URL_FAST_MENU_MANAGEMENT.'/delete/'.$data->menu_id.'">'.set_danger(lang('Admin.process.delete')).'</a>';
			}
		}
