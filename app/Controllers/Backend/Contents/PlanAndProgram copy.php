...
use App\Models\Backend\Contents\PlanAndProgramModel;
use App\Models\Backend\DatatableModel;

class PlanAndProgram extends BaseController {

	protected $table;
	protected $tableLang;
	protected $pageUrl;
	protected $filePath;
	protected $PlanAndProgramModel;
	protected $DatatableModel;

	public function __construct() {
		$this->table = 'plan_and_program';
		$this->tableLang = 'plan_and_program_lang';
		$this->pageUrl = ADMIN_URL_CONTENTS.'/'.ADMIN_URL_PLAN_AND_PROGRAM;
		$this->filePath = FILE_PATH_PLAN_AND_PROGRAM;
		$this->PlanAndProgramModel = new PlanAndProgramModel();
	...
	public function insert() {
		if ($this->request->isAJAX()) {
			if ($this->request->getMethod() == 'POST') {
				...
				$rules = [
					'form.status' => [
						'label' => lang('AdminContents.planAndProgram.general.status'),
						'rules' => 'required'
					],
					'lang.'.$this->defaultLangId.'.strategic_plan_name' => [
						'label' => lang('AdminContents.planAndProgram.general.name'),
						'rules' => 'required'
					],
					'form.strategic_plan_year' => [
						'label' => lang('AdminContents.planAndProgram.general.year'),
						'rules' => 'required'
					]
				];
				...
				if (isNotNull($file)) {
					$rules2 = [
						'strategic_plan_file' => [
							'label' => lang('AdminContents.planAndProgram.general.file.title'),
							'rules' => [
				...
				if (isNotNull($image)) {
					$rules3 = [
						'strategic_plan_image' => [
							'label' => lang('AdminContents.planAndProgram.general.image.title'),
							'rules' => [
				...
				if ($this->validate($rules)) {
					...
					if ($imageNameResult == NULL) {
						...
						if ($result !== FALSE) {
							...
							// Flash Data
							session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.add.planAndProgram', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
	...
	public function edit(int $strategic_plan_id) {
		$sql = $this->PlanAndProgramModel->planAndProgramInfoModel($strategic_plan_id);
		if (isNotNull($sql)) {

			$lang_array = [];
			$lang = $this->PlanAndProgramModel->planAndProgramLangModel($strategic_plan_id);
			if (isNotNull($lang)) {
				foreach ($lang as $row) {
							...
							if ($result !== FALSE) {
								...
								if (isNotNull($this->request->getVar('lang'))) {
									foreach ($this->request->getVar('lang') as $lang_id => $value) {
										...
										$langControlModel = $this->PlanAndProgramModel->planAndProgramLangControlModel($strategic_plan_id, $lang_id);
								...
								// Flash Data
								session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.edit.planAndProgram', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
