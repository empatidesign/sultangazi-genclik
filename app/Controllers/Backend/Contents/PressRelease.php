...
use App\Models\Backend\Contents\PressReleaseModel;
...
class PressRelease extends BaseController {
	...
	protected $pageUrl;
	protected $filePath;
	protected $PressReleaseModel;
	protected $DatatableModel;

	public function __construct() {
		$this->table = 'press_release';
		$this->tableLang = 'press_release_lang';
		$this->pageUrl = ADMIN_URL_CONTENTS.'/'.ADMIN_URL_PRESS_RELEASE;
		$this->filePath = FILE_PATH_PRESS_RELEASE;
	...
	public function insert() {
		if ($this->request->isAJAX()) {
			if ($this->request->getMethod() == 'POST') {
				...
				$rules = [
					'form.status' => [
						'label' => lang('AdminContents.pressRelease.general.status'),
						'rules' => 'required'
					],
					'lang.'.$this->defaultLangId.'.strategic_plan_name' => [
						'label' => lang('AdminContents.pressRelease.general.name'),
						'rules' => 'required'
					],
					'form.strategic_plan_year' => [
						'label' => lang('AdminContents.pressRelease.general.year'),
						'rules' => 'required'
					]
				];
				...
				if (isNotNull($file)) {
					$rules2 = [
						'strategic_plan_file' => [
							'label' => lang('AdminContents.pressRelease.general.file.title'),
							'rules' => [
				...
				if (isNotNull($image)) {
					$rules3 = [
						'strategic_plan_image' => [
							'label' => lang('AdminContents.pressRelease.general.image.title'),
							'rules' => [
				...
				if ($this->validate($rules)) {
					...
					if ($imageNameResult == NULL) {
						...
						if ($result !== FALSE) {
							...
							// Flash Data
							session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.add.pressRelease', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
	...
	public function edit(int $strategic_plan_id) {
		$sql = $this->PressReleaseModel->pressReleaseInfoModel($strategic_plan_id);
		if (isNotNull($sql)) {

			$lang_array = [];
			$lang = $this->PressReleaseModel->pressReleaseLangModel($strategic_plan_id);
			if (isNotNull($lang)) {
				foreach ($lang as $row) {
							...
							if ($result !== FALSE) {
								...
								if (isNotNull($this->request->getVar('lang'))) {
									foreach ($this->request->getVar('lang') as $lang_id => $value) {
										...
										$langControlModel = $this->PressReleaseModel->pressReleaseLangControlModel($strategic_plan_id, $lang_id);
								...
								// Flash Data
								session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.edit.pressRelease', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
