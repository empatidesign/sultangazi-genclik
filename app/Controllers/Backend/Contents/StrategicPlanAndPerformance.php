...
use App\Models\Backend\Contents\StrategicPlanAndPerformanceModel;
...
class StrategicPlanAndPerformance extends BaseController {
	...
	public function __construct() {
		$this->table = 'strategic_plan_and_performance';
		$this->tableLang = 'strategic_plan_and_performance_lang';
		$this->pageUrl = ADMIN_URL_CONTENTS.'/'.ADMIN_URL_STRATEGIC_PLAN_AND_PERFORMANCA;
		$this->filePath = FILE_PATH_STRATEGIC_PLAN_AND_PERFORMANCA;
	...
	public function insert() {
		if ($this->request->isAJAX()) {
			if ($this->request->getMethod() == 'POST') {
				...
				$rules = [
					'form.status' => [
						'label' => lang('AdminContents.strategicPlanAndPerformance.general.status'),
						'rules' => 'required'
					],
					'lang.'.$this->defaultLangId.'.strategic_plan_name' => [
						'label' => lang('AdminContents.strategicPlanAndPerformance.general.name'),
						'rules' => 'required'
					],
					'form.strategic_plan_year' => [
						'label' => lang('AdminContents.strategicPlanAndPerformance.general.year'),
						'rules' => 'required'
					]
				];
				...
				if (isNotNull($file)) {
					$rules2 = [
						'strategic_plan_file' => [
							'label' => lang('AdminContents.strategicPlanAndPerformance.general.file.title'),
							'rules' => [
				...
				if (isNotNull($image)) {
					$rules3 = [
						'strategic_plan_image' => [
							'label' => lang('AdminContents.strategicPlanAndPerformance.general.image.title'),
							'rules' => [
				...
				if ($this->validate($rules)) {
					...
					if ($imageNameResult == NULL) {
						...
						if ($result !== FALSE) {
							...
							// Flash Data
							session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.add.strategicPlanAndPerformance', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
	...
	public function edit(int $strategic_plan_id) {
		$sql = $this->StrategicPlanAndPerformanceModel->strategicPlanAndPerformanceInfoModel($strategic_plan_id);
		if (isNotNull($sql)) {

			$lang_array = [];
			$lang = $this->StrategicPlanAndPerformanceModel->strategicPlanAndPerformanceLangModel($strategic_plan_id);
			if (isNotNull($lang)) {
				foreach ($lang as $row) {
							...
							if ($result !== FALSE) {
								...
								if (isNotNull($this->request->getVar('lang'))) {
									foreach ($this->request->getVar('lang') as $lang_id => $value) {
										...
										$langControlModel = $this->StrategicPlanAndPerformanceModel->strategicPlanAndPerformanceLangControlModel($strategic_plan_id, $lang_id);
								...
								// Flash Data
								session()->setFlashdata('flashDataMessageSuccess', lang('AdminContents.result.edit.strategicPlanAndPerformance', [$this->request->getVar('lang['.$this->defaultLangId.'][strategic_plan_name]')]));
